...
private:
  ...
  // Expands cells currently marked free (0) outward into neighboring lethal (100)
  // cells that lie within `radius` meters, so the keep-in wall sits one robot
  // radius beyond the true operation/navigation polygon edges.
  void dilateFreeRegion(nav_msgs::msg::OccupancyGrid& occupancy_grid, double radius);
